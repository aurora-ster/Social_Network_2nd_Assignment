...
link_prediction_demo <- function(g, v1 = 1, v2 = 34, show_steps = FALSE, title = "Network") {
  ...
  # Choose layout: KK for small, FR for dense graphs like Facebook
  set.seed(42)
  ll <- if (vcount(g) > 100) layout_with_fr(g) else layout_with_kk(g)

  # Compute shortest path
  sp <- shortest_paths(g, from = v1, to = v2)$vpath[[1]]

  # Find neighbors and shared neighbors
  ...
  # Highlight neighbors
  if (length(nei_1) > 0) V(g)[nei_1]$color <- "gray80"
  if (length(nei_2) > 0) V(g)[nei_2]$color <- "gray80"
  if (length(common) > 0) V(g)[common]$color <- "deepskyblue"

  # Highlight focal nodes
  V(g)[c(v1, v2)]$color <- "darkorange2"
  V(g)[c(v1, v2)]$size <- 12
  V(g)[c(v1, v2)]$frame.color <- "black"


  # Highlight path
  if (length(sp) > 1) {
    path_edges <- E(g, path = sp)
  ...
  plot(
    g,
    layout = ll,
    vertex.label = if (vcount(g) > 100) NA else V(g),
    ...
    edge.arrow.size = if (is.directed(g)) 0.3 else 0,
    edge.curved = if (is.directed(g)) 0.2 else FALSE,
  ...
  adamic_adar <- if (length(deg_common) > 0) sum(1 / log(deg_common + 1e-10)) else 0
  ...
  # igraph built-in metrics (if undirected)
  if (!is.directed(g)) {
    cat("-- Built-in Similarity --\n")
    sim_jac <- similarity.jaccard(g, c(v1, v2))
    cat("Jaccard (igraph):", round(sim_jac[1, 2], 3), "\n")
    sim_aa <- similarity.invlogweighted(g, v1)[v2]
    cat("Adamic-Adar (igraph):", round(sim_aa, 3), "\n")
  }

  cat("\n")
}

link_prediction_demo(g_fb, 1, 34, title = "Copenhagen Facebook")
link_prediction_demo(g_calls, 1, 34, title = "Copenhagen Calls")
link_prediction_demo(g_sms, 1, 34, title = "Copenhagen SMS")
